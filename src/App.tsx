...
import { enableDevReset } from './utils/resetApp';
...
import { KawaiiGameScreen } from './components/screens/KawaiiGameScreen';
import { CountryOnboardingScreen } from './components/screens/CountryOnboardingScreen';
...
import { hasCompletedCountryOnboarding, saveUserCountry } from './utils/countryUtils';

type Screen = 'login' | 'resetPassword' | 'countryOnboarding' | 'home' | 'shop' | 'friends' | 'nearestPeople' | 'messages' | 'game' | 'dailyReward' | 'menu' | 'profile' | 'settings' | 'notifications' | 'help' | 'permissions';
...
export default function App() {
  ...
  // Enable dev reset function in console (development only)
  useEffect(() => {
    enableDevReset();
  }, []);
  ...
  // Game state
  // Note: In production, 'steps' will be synced from Google Fit
  const [gameState, setGameState] = useState(() => ({
    steps: 0, // Daily steps (will be synced from Google Fit)
    maxSteps: 10000, // Deprecated - will be replaced by dailyGoal
    totalSteps: 0, // Total lifetime steps for evolution (starts at Birth stage)
    coins: 0,
    level: 1,
  ...
  useEffect(() => {
    ...
    const initializeServices = async () => {
      ...
      // Request Google Fit permission and get initial steps
      try {
        ...
        if (healthPermission.granted) {
          ...
          // Only show error toast on actual Android device, not in development
          console.warn('Google Fit permission denied:', healthPermission.message);
          if (healthPermission.message && !healthPermission.message.includes('development')) {
            toast.error('Google Fit Access', {
    ...
    // Update steps from Google Fit every 5 minutes
  ...
  // When integrated with Google Fit:
  ...
  // - Sync 'steps' from Google Fit throughout the day
  ...
  const handleGoogleLogin = () => {
    ...
    // Check if user needs country onboarding
    if (!hasCompletedCountryOnboarding()) {
      setCurrentScreen('countryOnboarding');
      return;
    }
  ...
  const handleCountryOnboardingComplete = (countryCode: string) => {
    console.log('Country selected:', countryCode);
    saveUserCountry(countryCode);
    
    // After country onboarding, show welcome popup if first time, otherwise daily reward
    if (showWelcomePopup) {
      setCurrentScreen('home'); // Set to home but overlay welcome popup
    } else {
      setCurrentScreen('dailyReward');
  ...
  const renderCurrentScreen = () => {
    ...
    // Country onboarding
    if (currentScreen === 'countryOnboarding') {
      return (
        <CountryOnboardingScreen
          onComplete={handleCountryOnboardingComplete}
        />
      );
    }
      ...
      case 'home':
        return (
          <KawaiiHomeScreen
            ...
            onGameClick={() => setCurrentScreen('game')}

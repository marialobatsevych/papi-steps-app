...
// Event Configuration - TESTING MODE (20 minutes interval)
// Production values:
// - fly: 7200s (2h), poop: 28800s (8h), spider: 86400s (24h)
// - spawn_chance: 0.0008, 0.0005, 0.00015
export const EVENTS_CONFIG: EventsConfig = {
  fly: {
    reward_coins: 50,
    cooldown_seconds: 1200, // 20 minutes (testing)
    daily_max: 10, // Increased for testing
    spawn_chance_per_tick: 0.15, // 15% chance every minute
    duration_seconds: 10,
  },
  poop: {
    reward_coins: 20,
    ...
    cooldown_seconds: 1200, // 20 minutes (testing)
    daily_max: 10, // Increased for testing
    spawn_chance_per_tick: 0.12, // 12% chance every minute
  ...
  spider: {
    ...
    cooldown_seconds: 1200, // 20 minutes (testing)
    daily_max: 10, // Increased for testing
    spawn_chance_per_tick: 0.10, // 10% chance every minute
...
const TICK_INTERVAL = 60000; // 60 seconds (check every minute)
...
export function shouldSpawnPoopFromNeglect(
  ...
  if (isNeglected) {
    ...
    // If neglected for 20 minutes (testing mode), allow poop spawn
    // Production: 12 hours (12 * 60 * 60 * 1000)
    const neglectDuration = now - state.neglectTimer;
    const TWENTY_MINUTES = 20 * 60 * 1000; // Testing mode
    
    return neglectDuration > TWENTY_MINUTES && canSpawn('poop');
  } else {

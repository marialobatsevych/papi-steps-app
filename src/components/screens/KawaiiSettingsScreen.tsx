    ...
    <div className="w-full h-screen relative overflow-hidden bg-gradient-to-br from-[#FFF6E8] via-[#FFE8F0] to-[#F0E8FF]">
      ...
      <div className="absolute top-24 left-0 right-0 bottom-0 overflow-y-auto px-5 pb-6">
        <div className="max-w-[500px] mx-auto space-y-4">
          ...
          <div className="bg-white/90 backdrop-blur-xl rounded-[20px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-white/40">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-gradient-to-br from-[#B8E3FF] to-[#A6E3E9] rounded-full flex items-center justify-center">
                <span className="text-white">🌍</span>
              </div>
              <div className="flex-1">
                <h3 className="font-['Nunito'] font-semibold text-[#333]">
            ...
            <button
              ...
              className="w-full p-4 bg-white rounded-[16px] border-2 border-[#B8E3FF]/30 hover:border-[#B8E3FF] transition-all flex items-center justify-between mt-3"
            >
              <div className="flex items-center gap-3">
                <span className="text-3xl">{selectedCountryInfo?.flag || '🌍'}</span>
                <span className="font-['Nunito'] text-[#333]">
                  {selectedCountryInfo?.name || 'Select a country'}
                </span>
              </div>
              <span className="text-[#8E8E93]">{showCountryPicker ? '▲' : '▼'}</span>
            </button>
            
            {/* Country Picker Dropdown */}
            {showCountryPicker && (
              <div className="mt-3 bg-white rounded-[16px] shadow-xl max-h-[300px] overflow-hidden border-2 border-[#B8E3FF]/30">
                {/* Search */}
                <div className="p-3 border-b border-[#F5F5F5] sticky top-0 bg-white">
                  <input
                    ...
                    className="w-full px-4 py-2 bg-[#F5F5F5] rounded-full font-['Nunito'] text-sm focus:outline-none focus:ring-2 focus:ring-[#B8E3FF]"
                  />
                </div>
                ...
                <div className="overflow-y-auto max-h-[240px]">
                  {filteredCountries.map((country) => (
                    <button
                      ...
                      className={`w-full p-3 flex items-center gap-3 hover:bg-[#E8F5FF] transition-colors ${
                        selectedCountry === country.code ? 'bg-[#E8F5FF]' : ''
                      }`}
                    >
                      <span className="text-2xl">{country.flag}</span>
                      <span className="font-['Nunito'] text-[#333] text-left flex-1">
                        {country.name}
                      </span>
                      {selectedCountry === country.code && (
                        <span className="text-[#B8E3FF]">✓</span>
                      )}
                    </button>
                  ))}
            ...
            {/* Info message */}
            <div className="bg-[#B8E3FF]/10 rounded-[12px] p-3 mt-3">
              <p className="font-['Nunito'] text-xs text-[#666] leading-relaxed">
                🔒 Your country choice is used for Step Stars ⭐ rankings. We don't use GPS or location tracking.
              </p>
            </div>
          </div>
            ...
            {showNotifications && (
              <div className="space-y-4 mt-4 pt-4 border-t border-[#C8B8FF]/20">
                ...
                <div className="bg-[#C8B8FF]/10 rounded-[12px] p-3 mt-4">
                  <p className="font-['Nunito'] text-xs text-[#666] leading-relaxed">
                    💡 <span className="font-semibold">Note:</span> Notification permissions are managed through your device settings. Make sure to enable notifications for Papi Steps in your Android Settings.
                  </p>
                </div>
              </div>
            )}
